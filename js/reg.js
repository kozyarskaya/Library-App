...
// Функция вывода сообщений c ошибкой
...
function signUp() {
    if (inputUserName.value.trim() === "" || inputEmail.value.trim() === "" || 
        ...
        showAlertError(`Пожалуйста заполните все данные!`);
    } else if (inputPass1.value.trim() !== inputPass2.value.trim()) {
        showAlertError(`Пароли не совпадают!`);
    } else if (!inputEmail.value.trim().includes('@')) {
        showAlertError('Адрес электронной почты должен содержать @');
    } else {
    const input_data = { 
    ...
    .then(data => {
        ...
        if (data.ok) { 
            ...
            showAlert(`Аккаунт создан\nВаш ник и почта: ${inputUserName.value} ${inputEmail.value}`);
        } else {
            showAlertError(data.message); // Исправлено с "Massage" на "message"
        }
    })
    ...
    try {
        ...
        editForm.addEventListener('submit', (e) => {
            ...
            .then(data => {
                ...
                // Обновляем информацию о книге в DOM
                bookItem.innerHTML = ''; // Очищаем текущие данные

                const titleElement = document.createElement('h2');
                titleElement.textContent = data.title; // Используем новое название

                const authorElement = document.createElement('p');
                authorElement.textContent = `Автор: ${data.author}`; // Используем нового автора

                const dateElement = document.createElement('p');
                dateElement.textContent = `Дата: ${data.date}`; // Используем новую дату

                const previewElement = document.createElement('p');
                previewElement.textContent = data.preview; // Используем новый превью
