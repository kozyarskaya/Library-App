...
func (p *Provider) SelectArticle(id int) (api.ArticleRes, error) {
	...
	err1 := p.usersDB.QueryRow("SELECT name FROM users  WHERE id = $1", au_id).Scan(&art.Author)
...
func (p *Provider) SelectFullArticle(id int) (api.Article, error) {
	fmt.Println("in select art")
	var art api.Article
	err := p.articlesDB.QueryRow("SELECT id, title, text, author_id, date FROM articles WHERE id = $1 AND deleted = FALSE", id).Scan(&art.Id, &art.Title, &art.Text, &art.AuthorId, &art.Date)
	if err != nil {
