...
func (srv *Server) putArticle(e echo.Context) error {
	...
	if err != nil {
		return e.JSON(http.StatusBadRequest, echo.Map{"error": "Invalid user ID"})
	}
	fmt.Println("in putArticle, userId ok", userIdInt)

	// Извлекаем ID статьи из параметров запрос
	idStr := e.Param("id")
	id, err := strconv.Atoi(idStr)
	if err != nil {
	...
	fmt.Println("in putArticle, existingArticle id ok", existingArticle.AuthorId)
	//Проверка права редачить
	if existingArticle.AuthorId != userIdInt {
		fmt.Println("НЕТ ДОСТУПА РЕДАЧИТЬ")
		return e.JSON(http.StatusUnauthorized, Response{
			Ok:      "false",
			Message: "Нет доступа для редактироваания данной записи",
		})
	} else {
...
func (srv *Server) deleteArticle(e echo.Context) error {
	...
	fmt.Println("in deleteArticle, userId ok")
	// Извлекаем ID статьи из параметров запроса
	...
	// проверка, что userIdInt = id автора статьи, если нет- ошибка с сообщением
	article, _ := srv.uc.FetchFullArticleByID(id)
	if article.AuthorId != userIdInt {
		return e.JSON(http.StatusUnauthorized, Response{
			Ok:      "false",
			Message: "Нет доступа для удаления данной записи",
		})
	} else {
